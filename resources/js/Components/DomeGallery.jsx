...
export default function DomeGallery({
  ...
  // Slow auto-rotation effect
  useEffect(() => {
    let rafId;
    let lastTime = performance.now();
    const ROTATE_SPEED = 0.08; // degrees per frame, adjust for desired speed
    function animate() {
      const now = performance.now();
      const dt = now - lastTime;
      lastTime = now;
      // Only rotate if not dragging, not enlarging
      if (!draggingRef.current && !openingRef.current && !focusedElRef.current) {
        // Advance Y rotation
        rotationRef.current.y = wrapAngleSigned(rotationRef.current.y + ROTATE_SPEED * (dt / 16.67));
        applyTransform(rotationRef.current.x, rotationRef.current.y);
      }
