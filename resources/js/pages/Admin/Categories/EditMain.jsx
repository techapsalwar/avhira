...
export default function EditMainCategory({ mainCategory }) {
    const { data, setData, put, processing, errors } = useForm({
        name: mainCategory.name || '',
        description: mainCategory.description || '',
        display_order: mainCategory.display_order || 0,
        is_active: mainCategory.is_active ?? true,
    });

    const handleSubmit = (e) => {
        e.preventDefault();
        put(`/admin/categories/main/${mainCategory.id}`);
    };

    return (
        <AdminLayout title="Edit Main Category">
            <Head title={`Edit ${mainCategory.name} - Admin`} />
                ...
                <div className="bg-white rounded-2xl shadow-md p-8">
                    <div className="flex items-center justify-between mb-6">
                        <div>
                            ...
                            <p className="text-gray-600 mt-1">Update main category details</p>
                        </div>
                        <Link
                    ...
                    <form onSubmit={handleSubmit} className="space-y-6">
                        ...
                        <div className="flex items-center justify-end gap-3 pt-6 border-t">
                            ...
                            >
                                {processing ? 'Updating...' : 'Update Main Category'}
                            </button>
                        </div>
                    </form>
                </div>
